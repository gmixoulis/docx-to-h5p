#!/usr/bin/env python3
"""
H5P Quiz Combiner with Crossword, Multiple Choice, True/False, and Library Support
Combines multiple JSON files from activities_module and creates separate H5P files for:
1. Multiple Choice questions (using H5P.QuestionSet)
2. Crossword puzzles (using H5P.Crossword)
3. True/False questions (using H5P.TrueFalse)
"""
...
class H5PQuizCombiner:
    ...
    def detect_question_type(self, json_data: Dict) -> QuestionType:
        """
        Detect whether a JSON file is multiple choice, crossword, or true/false.
        ...
        if "question" in json_data and "correct" in json_data:
            # True/False has a boolean 'correct' field
            if isinstance(json_data["correct"], bool):
    ...
    def find_json_in_modules(self, modules_folder: str) -> Dict[QuestionType, List[Dict]]:
        """
        Recursively search for JSON files in activities_module directories.
        Automatically categorizes them by type.

        Args:
            modules_folder: Path to activities_module folder
        ...
        print(f"\nðŸ” Searching for JSON files in {modules_folder}...\n")

        # Walk through all subdirectories
        for root, dirs, files in os.walk(modules_folder):
            for file in files:
                if file.endswith('.json'):
                    # Skip language files and library.json
                    if file in ['el.json', 'es.json', 'library.json']:
                        continue
                    ...
                    try:
                        ...
                        relative_path = os.path.relpath(file_path, modules_folder)
                        print(f"  ðŸ“‹ {relative_path}: {type_name}")

                    except Exception as e:
                        print(f"  âŒ Error loading {file_path}: {e}")
    ...
    def categorize_questions(self, question_files: List[str]) -> Dict[QuestionType, List[Dict]]:
        ...
        for json_file in question_files:
            try:
                ...
                type_name = q_type.value.replace('_', ' ').title()
                print(f"  ðŸ“‹ {os.path.basename(json_file)}: {type_name}")

            except Exception as e:
                print(f"  âŒ Error loading {json_file}: {e}")
                categorized[QuestionType.UNKNOWN].append({
                    'filename': json_file,
                    'data': None,
                    'error': str(e)
    ...
    def create_true_false_h5p(
        ...
        """
        # Update metadata
        self.true_false_metadata["title"] = title
    ...
    def _create_h5p_file(
        ...
        if self.library_json_path and os.path.exists(self.library_json_path):
            ...
            print(f"  ðŸ“š Added library.json")
        ...
        if self.language_files_folder and os.path.exists(self.language_files_folder):
            for lang_file in ['el.json', 'es.json']:
                ...
                if os.path.exists(src_path):
                    ...
                    print(f"  ðŸŒ Added language file: {lang_file}")
    ...
    def process_all_questions(
        ...
        true_false_title: str = "True/False Quiz",
        ...
        Args:
            ...
            true_false_title: Title for true/false quiz
        ...
        if categorized[QuestionType.MULTIPLE_CHOICE]:
            print(f"\nðŸŽ¯ Creating Multiple Choice quiz...")
        ...
        if categorized[QuestionType.TRUE_FALSE]:
            print(f"\nâœ”ï¸âŒ Creating True/False quiz...")
            tf_filename, tf_count = self.create_true_false_h5p(
                categorized[QuestionType.TRUE_FALSE],
                f"{output_prefix}_true_false",
                true_false_title,
                images_folder,
                pass_percentage
            )
    ...
    def process_activities_modules(
        self,
        modules_folder: str,
        output_prefix: str = "quiz",
        mc_title: str = "Multiple Choice Quiz",
        ...
        """
        Find and process all JSONs in activities_module folders.

        Args:
            modules_folder: Path to activities_modules folder
            output_prefix: Prefix for output files
            mc_title: Title for multiple choice quiz
            true_false_title: Title for true/false quiz
            crossword_title: Title for crossword
        ...
        Returns:
            Dictionary mapping question type to (filename, count) tuples
        """
        # Find and categorize JSONs from modules
        categorized = self.find_json_in_modules(modules_folder)
...
def main():
    ...
    if len(sys.argv) < 2:
        print("H5P Quiz Combiner with True/False, Multiple Choice, Crossword, and Library Support")
        print("=" * 80)
        print("\nUsage:")
        print("  python h5p_combiner_enhanced.py <source> [images_folder] [lang_folder] [library_json] [output_prefix] [options]")
        print()
        print("Arguments:")
        print("  source          : Folder containing JSON files OR activities_modules folder (required)")
        print("  images_folder   : Folder containing images (optional)")
        print("  lang_folder     : Folder containing el.json and es.json (optional)")
        print("  library_json    : Path to library.json file (optional)")
        print("  output_prefix   : Prefix for output files (default: 'quiz')")
        print()
        print("Examples:")
        print("  # Process activities_modules folder:")
        print("  python h5p_combiner_enhanced.py activities_modules/")
        print()
        print("  # Process specific JSON folder with all options:")
        print("  python h5p_combiner_enhanced.py questions/ images/ languages/ library.json my_quiz")
        print()
        print("File structure:")
        print("  activities_modules/")
        print("    â””â”€â”€ Module-1/")
        print("        â”œâ”€â”€ questions/")
        print("        â”‚   â”œâ”€â”€ question1.json (multiple choice)")
        print("        â”‚   â”œâ”€â”€ question2.json (true/false)")
        print("        â”‚   â””â”€â”€ crossword.json")
        print("        â””â”€â”€ ...")
        print()
        print("  languages/")
        print("    â”œâ”€â”€ el.json  (Greek translations)")
        print("    â””â”€â”€ es.json  (Spanish translations)")
        print()
        print("This will create separate H5P files for:")
        print("  â€¢ Multiple Choice questions")
        print("  â€¢ True/False questions")
        print("  â€¢ Crossword puzzles")
        return
    ...
    library_json = sys.argv[4] if len(sys.argv) > 4 and sys.argv[4] != '' else None
    output_prefix = sys.argv[5] if len(sys.argv) > 5 else "quiz"

    # Check if source is an activities_modules folder or regular folder
    is_activities_modules = 'activities' in source_path.lower() or 'module' in source_path.lower()

    print("=" * 80)
    print("ðŸŽ“ H5P Quiz Combiner with True/False, Multiple Choice, and Crossword Support")
    print("=" * 80)

    if lang_folder:
        print(f"\nðŸŒ Language folder: {lang_folder}")
    if library_json:
        print(f"\nðŸ“š Library file: {library_json}")

    # Create combiner with all options
    ...
    if is_activities_modules and os.path.isdir(source_path):
        results = combiner.process_activities_modules(
            modules_folder=source_path,
            output_prefix=output_prefix,
            mc_title="Multiple Choice Quiz",
            true_false_title="True/False Quiz",
            crossword_title="Crossword Puzzle",
            images_folder=images_folder
        )
    else:
        # Process regular folder or file list
        if os.path.isdir(source_path):
            json_pattern = os.path.join(source_path, "*.json")
            json_files = glob.glob(json_pattern)
            json_files = [f for f in json_files 
                         if os.path.basename(f) not in ['el.json', 'es.json', 'library.json']]
        else:
            json_files = glob.glob(source_path)

        if not json_files:
            print(f"\nâŒ No JSON files found in {source_path}")
            return
    ...
    if results:
        ...
        if 'true_false' in results:
            ...
            size = os.path.getsize(filename)
            print(f"  âœ”ï¸âŒ True/False Quiz:")
            print(f"     â€¢ File: {filename}")
            print(f"     â€¢ Questions: {count}")
            print(f"     â€¢ Size: {size:,} bytes\n")
        ...
        if 'crossword' in results:
            ...
            size = os.path.getsize(filename)
            print(f"  ðŸ§© Crossword Puzzle:")
            print(f"     â€¢ File: {filename}")
            print(f"     â€¢ Words: {count}")
            print(f"     â€¢ Size: {size:,} bytes\n")

        print("ðŸŽ“ Ready to upload to your LMS (Moodle, WordPress, Canvas, etc.)!")

        features = []
        if lang_folder:
            features.append("ðŸŒ Multilingual support")
        if library_json:
            features.append("ðŸ“š Library metadata")
